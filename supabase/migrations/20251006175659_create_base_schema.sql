/*
  # Create Base Schema for Security Role Request System

  1. New Tables
    - `security_role_requests` - Main request table for security role access
    - `security_role_selections` - Role selections and configurations
    - `security_areas` - Security area assignments per request
    - `request_approvals` - Approval workflow tracking
    - `role_catalog` - Master catalog of available roles
  ...
  3. Notes
    - This is the foundational schema for the security role request system
    - All tables use UUID primary keys
    - Timestamps track creation and updates
*/

-- Create security_role_requests table
...
-- Create security_role_selections table
...
-- Policies for security_role_requests
CREATE POLICY "Allow public read access to requests"
  ON public.security_role_requests
  FOR SELECT
  USING (true);
...
-- Policies for security_role_selections
...
-- Policies for security_areas
CREATE POLICY "Allow public read access to areas"
  ON public.security_areas
  FOR SELECT
  USING (true);
...
-- Policies for request_approvals
